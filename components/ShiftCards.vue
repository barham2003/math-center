...
const { data: shifts, error } = await useFetch<IAttendance[]>(
  () => {
    return apiURL.value;
  },
  { key: "shifts", watch: [day] },
);
...
function isLate(shift: any) {
  const isLate =
    convertTimeToNumber(
      new Date(shift.attendance?.checkinTime).toTimeString().split(" ")[0],
    ) +
      600 >
    shift.from;
  return isLate;
...
<template>
  <div v-if="shifts.length > 0 && !error" :class="props.class">
    ...
    >
        ...
        <NuxtLink :to="`/admin/dashboard/tutors/${shift.tutor._id}`">
          <div class="flex items-center gap-2">
            <Avatar class="h-8 w-8">
              <AvatarImage
                v-if="shift.tutor.picture"
                :src="shift.tutor.picture"
                alt="user pic"
                class="rounded-md"
              />
      ...
      <CardContent class="flex flex-col py-4">
        ...
        <div>
          <div class="h-[3.5rem]">
            <p v-if="shift.attendance" class="flex items-center gap-2">
              <Check :size="18" />
              Checked in at
              {{ new Date(shift.attendance.checkinTime).toLocaleTimeString() }}
            </p>
            <p
              v-if="shift.attendance && shift.attendance.checkoutTime"
          ...
          <Button
            v-if="!shift.attendance"
            class="w-fit"
            variant="outline"
            @click="checkinAttendance(shift._id)"
          >
            <LogIn class="text-green-600" />
            {{ isTutor ? "Check in" : "Still" }}
          </Button>
          <Button
            v-else-if="shift.attendance && !shift.attendance.checkoutTime"
            class="w-fit"
            variant="outline"
  ...
  <Card v-else class="flex h-[18rem] items-center justify-center text-center">
