...
const { data, error } = await useFetch<IUser[]>("/api/tutors", { key: "tutors" });

const formattedData = data.value.map((tutor) => {
  return { name: tutor.name, email: tutor.email, "Joined At": new Date(tutor.createdAt).toLocaleDateString() }
